...
async def search_stocks(
    ...
    try:
        ...
        logger.error(f"[X] 搜索股票失败: market={market}, q={q}, error={e}", exc_info=True)
        raise HTTPException(
...
async def get_stock_info(
    ...
    try:
        ...
        logger.error(f"[X] 获取股票信息失败: market={market}, code={code}, error={e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_stock_quote(
    ...
    try:
        ...
        logger.error(f"[X] 获取股票行情失败: market={market}, code={code}, error={e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_stock_daily_quotes(
    ...
    try:
        ...
        logger.error(f"[X] 获取历史K线失败: market={market}, code={code}, error={e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
