...
def test_frontend_backend_integration():
    ...
    try:
        ...
        if submit_response.status_code != 200:
            print(f"[X] 任务提交失败: {submit_response.status_code}")
        ...
        if not submit_data.get("success"):
            print("[X] 响应中缺少 success 字段")
            return False
        
        if not submit_data.get("data"):
            print("[X] 响应中缺少 data 字段")
            return False
        
        if not submit_data["data"].get("task_id"):
            print("[X] 响应中缺少 task_id 字段")
            return False
    ...
    try:
        ...
        if status_response.status_code != 200:
            print(f"[X] 状态查询失败: {status_response.status_code}")
        ...
        if not status_data.get("success"):
            print("[X] 状态响应中缺少 success 字段")
            return False
        
        if not status_data.get("data"):
            print("[X] 状态响应中缺少 data 字段")
            return False
    ...
    try:
        ...
        if tasks_response.status_code != 200:
            print(f"[X] 任务列表查询失败: {tasks_response.status_code}")
