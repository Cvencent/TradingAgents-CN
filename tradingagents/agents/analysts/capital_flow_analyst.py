...
请使用中文，基于真实数据进行分析。"""
        ...
        if should_use_tool_call_handler(llm):
            ...
            return {
                ...
                "capital_flow_request_prompt": full_prompt_with_tools  # 🔥 保存包含工具结果的完整prompt
            }
        else:
            # 非Google模型的处理逻辑
...
报告必须基于工具返回的真实数据进行分析，包含具体的资金流向数值，长度不少于800字，使用中文撰写。"""
                        ...
                        return {
                            "messages": [result],
                            "capital_flow_report": report,
                            "capital_flow_request_prompt": full_prompt_with_response  # 🔥 保存包含AI回复的完整prompt
                        }
                    else:
...
报告必须基于工具返回的真实数据进行分析，包含具体的资金流向数值，长度不少于800字，使用中文撰写。"""
                    ...
                    return {
                        "messages": [result],
                        "capital_flow_report": report,
                        "capital_flow_request_prompt": full_prompt_with_error  # 🔥 保存包含工具调用失败的完整prompt
                    }
            else:
                # 没有工具调用，直接使用LLM返回的内容
