...
def _get_company_name_for_market_trend(ticker: str, market_info: dict) -> str:
    """
    为市场分析师获取公司名称
        ...
        if market_info['is_china']:
            ...
            if stock_info and "股票名称:" in stock_info:
                ...
                try:
                    ...
                    logger.error(f"[X] [市场分析师] 降级方案也失败: {e}")

                logger.error(f"[X] [市场分析师] 所有方案都无法获取股票名称: {ticker}")
...
def create_market_trend_analyst(llm, toolkit):
    ...
    def market_trend_analyst_node(state):
        ...
        logger.debug(f"📈 [DEBUG] 现有市场趋势报告: {state.get('market_trend_report', 'None')}")
        ...
        # 获取公司名称
        company_name = _get_company_name_for_market_trend(ticker, market_info)
        logger.debug(f"📈 [DEBUG] 公司名称: {ticker} -> {company_name}")

        # 统一使用 get_market_trend_and_sector 工具
        # 该工具用于获取大盘走势和行业走势数据
        logger.info(f"📊 [市场分析师] 使用大盘走势和行业走势工具")
        tools = [toolkit.get_market_trend_and_sector]

        # 从MongoDB获取配置，如果不存在则使用默认配置
        config_manager = get_agent_config_manager()

        # 默认提示模板
        default_system_prompt = """你是一位专业的市场分析师，专注于分析A股大盘走势、股票所属行业走势和整个市场行情。
...
⚠️ 重要工作流程：
1. 如果消息历史中没有工具结果，立即调用 get_market_trend_and_sector 工具
   - ticker: {ticker}
...
## 📈 大盘走势分析
[在这里分析上证指数、深证指数、创业板指数等大盘走势，提供具体数值和趋势判断]

## 🏭 行业走势分析
[在这里分析股票所属行业的整体走势，包括行业指数、龙头股表现等]

## 💭 市场环境分析
[在这里分析宏观经济环境、政策环境、市场情绪等整体市场因素]
...
- 不要在标题中使用"市场趋势分析报告"等自创标题
- 如果你有明确的市场面投资建议（买入/持有/卖出），请在投资建议部分明确标注
...
请使用中文，基于真实数据进行分析。"""
        ...
        if config_manager:
            # 尝试从MongoDB获取提示模板
            db_prompt_template = config_manager.get_prompt_template("market_trend")
        ...
        try:
            ...
            logger.info(f"📊 [市场分析师] 构建完整请求prompt，长度: {len(full_request_prompt)}")
        except Exception as e:
            logger.warning(f"⚠️ [市场分析师] 构建完整prompt失败: {e}")
            # 降级：手动构建prompt
            full_request_prompt = f"""=== System Message ===
你是一位专业的市场趋势分析师...

=== Human Message ===
请分析股票 {ticker} ({company_name}) 的市场趋势...
"""
        ...
        if should_use_tool_call_handler(llm):
            ...
            analysis_prompt_template = GoogleToolCallHandler.create_analysis_prompt(
                ...
                analyst_type="市场趋势分析",
                specific_requirements="重点关注大盘走势、行业走势、宏观经济环境等市场指标。"
            )
            ...
            has_tool_call_in_content = ('"ticker"' in content_str or '"stock_code"' in content_str) and \
                                       ('get_market_trend_and_sector' in content_str or 'get_stock_sentiment' in content_str or
                ...
                try:
                    ...
                    if json_matches:
                        ...
                        for json_str in json_matches:
                            try:
                                ...
                                for tool in tools:
                                    ...
                                    if 'get_market_trend_and_sector' in tool_name:
                                        try:
                                            # 🔧 修复：判断工具类型，普通函数直接调用，有invoke方法的用invoke
                                            ...
                                            logger.error(f"[X] [市场分析师] 工具执行失败: {tool_error}")
                        ...
                        if tool_messages:
                            ...
                            analysis_prompt = f"""现在请基于上述工具获取的数据，生成详细的市场趋势分析报告。
...
请按照专业格式输出完整的市场趋势分析报告，包括：
1. 股票基本信息
2. 大盘走势分析（上证、深证、创业板等指数走势）
3. 行业走势分析（股票所属行业的整体走势）
4. 市场环境分析（宏观经济、政策环境、市场情绪）
5. 投资建议（买入/持有/卖出，目标价位，止损位等）

报告必须基于工具返回的真实数据进行分析，包含具体的大盘指数和行业指数数值，长度不少于800字，使用中文撰写。"""
