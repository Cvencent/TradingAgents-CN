...
class TradingAgentsGraph:
    ...
    def propagate(self, company_name, trade_date, progress_callback=None, task_id=None):
        ...
        # 获取全局超时配置（默认10分钟）
        global_timeout = self.config.get("analysis_timeout", 600)
        logger.info(f"⏱️ [全局超时] 分析任务总超时时间: {global_timeout}秒")
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(_execute_analysis)
                try:
                    result = future.result(timeout=global_timeout)
                    logger.info(f"✅ [分析完成] 分析成功完成，耗时: {time.time() - total_start_time:.2f}秒")
                    return result
                    ...
                    timeout_decision = {
                        ...
                        'risk_score': 1.0,
                        'target_price': None,
                        'reasoning': f'分析超时（{global_timeout}秒），无法完成分析。请检查网络连接或降低研究深度。',
                        'model_info': 'Timeout',
